...
| 6 | Signing config (project settings) | 🟢 GREEN | bundle id `com.mousegestures.MouseGestures`; `ENABLE_HARDENED_RUNTIME=YES` (Release); entitlements file present (apple-events only, sandbox off by design); exportOptions `method=developer-id`, `teamID=2RZ7SBH74J` aligned with pbxproj |
...
- **Unblock:**
  1. Complete Apple Developer Program enrollment for team `2RZ7SBH74J`.
  ...
  4. Local signed build path: `./scripts/package_dmg.sh --identity "Developer ID Application: … (2RZ7SBH74J)"`.
...
- **Who holds** the Developer ID cert + App Store Connect / notarization access,
  and confirm **team `2RZ7SBH74J`** is the publishing account.
